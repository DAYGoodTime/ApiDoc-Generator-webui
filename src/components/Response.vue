<template >
  <el-divider/>
    <div v-for="(item,index) in store.Response_length[props.index]" >
      <el-descriptions
          class="margin-top"
          :column="3"
          :size="size"
      ...
      >
        ...
        <template #extra v-if="index===0">
          <el-input-number v-model="store.Response_length[props.index]" :min="1" @change="(...event) =>{handelChange(event, props.index)}"/>
        </template>
